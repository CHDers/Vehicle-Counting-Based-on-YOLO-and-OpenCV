...
model = YOLO(ROOT_PATH / 'conf/model/yolov8n.pt')
...
cap = cv2.VideoCapture(os.path.abspath(r"../datasets/V10603-140519.mp4"))

mask = cv2.imread(r'../conf/mask.png', cv2.IMREAD_GRAYSCALE)
mask_for_detection = cv2.imread(r'../conf/mask.png')
...
while True:
    ...
    for result in resultsTracker:
        ...
        cv2.rectangle(mask_added, (int(x1), int(y1)),
                      (int(x2), int(y2)), (255, 0, 0), 1)
        cv2.putText(mask_added, str(int(Id)), (int(x1), int(y1) - 3),
        ...
        cv2.putText(mask_added, "Cars Count: " + str(len(totalCount)), (50, 50),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, 255, 2, cv2.LINE_AA, False)
